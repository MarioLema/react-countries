...
class DetailSection extends Component {
    ...
    neighboursButtons(){
      ...
      for(let i = 0; i < neighbours.length; i++){
        for(let country of countries){
          ...
          neighArray.push(
            <button className="mini-button" alphacode={country.alpha3Code} onClick={this.props.detailMethod} key={country.alpha3Code} style={{background: style.background, color: style.color}}>{country.name}</button>)
        }
      }
      return neighArray;
    }
    render() {
      ...
      return (
        ...
        <button className="mini-button return-button" onClick={this.props.returnMethod} style={{background: style.background, color: style.color}}><i className="fas fa-long-arrow-alt-left"></i> Back</button>
